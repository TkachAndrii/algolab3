...
private:
    ...
    void _PrintTree(SplayNode* root, int depth = 0) const {
        ...
        std::cout << std::string(4 * depth, ' ') << "- " << root->data.real << " + " << root->data.imag << "i" << std::endl;
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            case 1: {
                double real = getValidatedNumber("Enter real part: ");
                double imag = getValidatedNumber("Enter imaginary part: ");
                tree.Insert(Complex(real, imag));
                break;
            }
            case 2: {
                double real = getValidatedNumber("Enter real part: ");
                double imag = getValidatedNumber("Enter imaginary part: ");
                if (tree.Search(Complex(real, imag))) {
